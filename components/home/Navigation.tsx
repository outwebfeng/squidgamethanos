    ...
    <header className='bg-frosted-glass sticky left-0 top-0 z-50 flex h-[64px] px-5 blur-[60%] filter lg:px-0'>
      <nav className='mx-auto flex max-w-pc flex-1 items-center justify-between bg-[#ffffff] px-4'>
        <div className='flex items-center gap-x-2'>
          <Link prefetch={false} className='flex items-center gap-x-2 hover:opacity-80' href='/' title={t('title')}>
            <img
              src='/favicon.ico'
              alt={t('logoAlt')}
              title={t('logoTitle')}
            ...
            />
            <span className='text-lg font-semibold text-[rgb(54,198,255)]'>{t('logoTitle')}</span>
          </Link>
        </div>
        <QuickMenu />
